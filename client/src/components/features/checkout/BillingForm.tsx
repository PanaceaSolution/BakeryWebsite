...
// Schema this will be migrated later to separate file just for testing now
...
export function BillingForm() {
  ...
  return (
    <div
      className={`
        absolute top-[580px] left-[91px] 
        w-[973px] h-[1354px] 
        bg-white p-6 sm:p-8 md:p-10 
        rounded-[10px] shadow-md 
        opacity-100
        xl:static xl:w-full xl:max-w-4xl xl:h-auto 
        md:relative md:mx-auto md:top-10
      `}
    >
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 font-poppins">
          {/* Basic Fields */}
          {[
            { name: "senderName", label: "Sender’s Full Name", required: true, type: "text" },
            { name: "senderPhone", label: "Sender’s Phone Number", required: true, type: "tel" },
            { name: "senderEmail", label: "Sender’s Email", required: false, type: "email" },
            { name: "deliveryLocation", label: "Delivery Location", required: true, type: "text" },
            { name: "receiverPhone1", label: "Receiver Contact Number (1st)", required: true, type: "tel" },
            { name: "receiverPhone2", label: "Receiver Contact Number (2nd) (Optional)", required: false, type: "tel" },
          ...
          <FormField control={form.control} name="message" render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="E.g. Happy Birthday my Love!" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )} />
          ...
          {/* Upload Photo */}
          <FormField control={form.control} name="uploadPhoto" render={({ field }) => (
            <FormItem>
          ...
          <FormField control={form.control} name="orderNotes" render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea placeholder="Special notes for delivery..." rows={3} {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )} />
